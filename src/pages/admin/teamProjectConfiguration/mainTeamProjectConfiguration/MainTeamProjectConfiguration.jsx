...
const MainTeamProjectConfiguration = () => {
  ...
  const [isSuccessDeleteModalOpen, setIsSuccessDeleteModalOpen] =
    useState(false);

  const deleteTeamProjectData = async () => {
    try {
      setLoading(true);
      await axios.delete(`http://103.82.93.38/api/v1/team_project/${uuid}`, {
        headers: {
          Authorization: token,
        },
      });
      ...
      render: (record) => (
        <Space size="small">
          ...
          <Button
            ...
            type="primary"
            size="small"
            onClick={() => isDeleteButtonClicked(record)}
            ghost
          >
  ...
  return (
    <div>
      <div>
        <Row gutter={[16, 8]}>
          <Col xs={24} md={20} lg={21} xl={8} xxl={6}>
            <SearchBox onSearch={handleSearch} />
          </Col>
          <Col xs={6} md={4} lg={3} xl={2} xxl={2}>
            <CountButton className="count-button" onCount={handleCount} />
          </Col>
          <Col xs={18} md={12} lg={12} xl={4} xxl={3}>
            <SortButton
