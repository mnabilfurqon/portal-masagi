...
import { Col, Row, Table, DatePicker, Input, Button, Flex, Avatar, Spin, Card } from 'antd'
...
const ReportConfiguartion = () => {
  ...
  const getSummaryCompany = async () => {
    try {
      setLoading(true);
      const response = await axios.get(`http://103.82.93.38/api/v1/attendance/summary_company`,
        {
      ...
      setLoading(false);
      // console.log("Summary company", response.data)
  ...
  const getReportCompany = async () => {
    try {
      ...
      setLoading(false);
      setReportCompany(response.data)
    //   console.log("report company", reportCompany)
      // console.log(response);
    } catch (error) {
      console.log("Error", error);
      setLoading(false);
  ...
  const dataSource = reportCompany?.map(item => {
      return {
          ...
          absents: item.absent,
          official_travels: item.official_travel,
          overtimes: item.overtime,
          leaves: item.leaves,
          permits: item.permit,
      }
  })
        ...
        <Row gutter={[16, 16]}>
            <Col xs={12} sm={8} md={8} lg={6} xl={4} xxl={4}>
                <Card className='summary-button'>
                    ...
                    <span style={{ fontSize: 14, marginTop: 10, paddingBottom: 0 }}>
                        Total Presents
                    </span>
                    <h2  style={{ fontSize: 24, margin: 0, fontWeight: "bold", }}>
                        {totalPresent}
                    </h2>
                    </center>
                </Card>
